...
public class VolController {
	...
	public Vol postVol(@RequestBody Vol vol) {
		...
		Vol _vol = repository.save(new Vol(vol.getDatedepart(),vol.getDatedarrive(),vol.getLieudepart(),vol.getAgance(),vol.getAvion(),vol.getLieudarrive() ,vol.getAge()));
		return _vol;
	}
	...
	public ResponseEntity<Vol> updateVols(@PathVariable("id") long id, @RequestBody Vol vol) {
		...
		if (voldata.isPresent()) {
			...
			_vol.setAge(vol.getAge());
			_vol.setActive(vol.isActive());
